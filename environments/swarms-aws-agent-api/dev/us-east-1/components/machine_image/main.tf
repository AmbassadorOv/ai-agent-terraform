...
resource "aws_instance" "fastapi_server_test_instance" {
  ...
  # Enforce IMDSv2 (metadata http_tokens = "required") to mitigate SSRF credentials extraction risks.
  metadata_options {
    http_endpoint = "enabled"
    http_tokens   = "required"
  }

  # Enforce root storage volume encryption for security compliance.
  root_block_device {
    encrypted = true
  }
