...
class MetaAutoscaler:
    ...
    def compute_target_scale(self, state: ScalingState):
        ...
        # Monitor swarm and send alerts if needed
        self.auto_orch.monitor_swarm(target)
...
def train_loop_per_worker(config):
    ...
    for epoch in range(config.get("epochs", 5)):
        ...
        # Simulated Scaling State for monitoring
        state = ScalingState(
            crp_inflight=1000,
            router_entropy=entropy.item(),
            active_agents=500 + epoch * 10, # Simulate crossing threshold
            queue_depth=2000,
            avg_latency_ms=200,
            cluster_utilization=0.7
        )
        autoscaler.compute_target_scale(state)
